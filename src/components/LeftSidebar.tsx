...
export default function LeftSidebar({ isOpen, onClose, activeItem, onItemClick }: LeftSidebarProps) {
    ...
    return (
      <div key={item.id}>
        ...
        >
          ...
          {hasChildren && (
            isExpanded ? (
              <ChevronDown className={`ml-2 h-3 w-3 ${isActive ? 'text-blue-600' : 'text-gray-500'}`} />
            ) : (
              <ChevronRight className={`ml-2 h-3 w-3 ${isActive ? 'text-blue-600' : 'text-gray-500'}`} />
            )
          )}
        </Button>
  ...
  return (
    <TooltipProvider>
        <aside className="fixed left-12 w-64 h-[calc(100vh-45px-32px)] z-30 bg-white border-r border-gray-200 shadow-sm" style={{ top: '45px' }}>
